...
def _mix_s_d_info(
    ...
    if d0 is not None:
        if isinstance(use_d_keys, dict):
            x_d_info = {k: x_s_info[k] + (v - d0[k]) * use_d_keys.get(k, 1) for k, v in x_d_info.items()}
        else:
...
def _fix_exp_for_x_d_info(x_d_info, x_s_info, delta_eye=None, drive_eye=True):
    ...
    if delta_eye is not None and drive_eye:
        ...
        alpha[_eye] = 1
        alpha = alpha.reshape(1, -1)
        x_d_info['exp'] = (delta_eye + x_s_info['exp']) * alpha + x_d_info['exp'] * (1 - alpha)
...
class MotionStitch:
    ...
    def set_Nd(self, N_d=-1):
        ...
        if self.drive_eye and self.delta_eye_arr is not None:
            ...
            self.delta_eye_idx_list = _set_eye_blink_idx(N, len(self.delta_eye_arr), self.delta_eye_open_n)
    ...
    def _set_scale_ratio(self, scale_ratio=1):
        ...
        if isinstance(self.use_d_keys, dict):
            self.use_d_keys = {
                k: v * (scale_ratio if k in {'exp', 'pitch', 'yaw', 'roll'} else 1) for k, v in self.use_d_keys.items()
            }
        else:
    ...
    def __call__(self, x_s_info, x_d_info, **kwargs):
        ...
        delta_eye = 0
        if self.drive_eye and self.delta_eye_arr is not None:
            delta_eye = self.delta_eye_arr[self.delta_eye_idx_list[self.idx % len(self.delta_eye_idx_list)]][None]
