...
class TRTWrapper:
    def __init__(
        ...
        assert self.engine
        self.buffer = OrderedDict()  # type: ignore
        self.output_allocator_map = OrderedDict()  # type: ignore
    ...
    def setup(self, input_data: dict = {}) -> None:
        for name, value in self.buffer.items():
            ...
            if device_buffer is not None and device_buffer != 0 and name not in self.output_allocator_map:
        ...
        self.n_input = sum(
            [self.engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT for name in self.tensor_name_list],
        )
    ...
    def __del__(self):
        if hasattr(self, 'buffer') and self.buffer is not None:
            for _, device_buffer, _ in self.buffer.values():
                if device_buffer is not None and device_buffer != 0 and cudart is not None:
                    try:
                        checkCudaErrors(cudart.cudaFree(device_buffer))
                    except TypeError:
                        pass
