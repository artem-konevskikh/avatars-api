...
def _transform_img(img, M, dsize, flags=CV2_INTERP, borderMode=None):
    """Conduct similarity or affine transformation to the image, do not do
    border operation!
...
def parse_pt2_from_pt101(pt101, use_lip=True):
    """Parsing the 2 points according to the 101 points, which cancels the
    roll."""
...
def parse_pt2_from_pt106(pt106, use_lip=True):
    """Parsing the 2 points according to the 106 points, which cancels the
    roll."""
...
def parse_pt2_from_pt203(pt203, use_lip=True):
    """Parsing the 2 points according to the 203 points, which cancels the
    roll."""
...
def parse_pt2_from_pt68(pt68, use_lip=True):
    """Parsing the 2 points according to the 68 points, which cancels the
    roll."""
...
def parse_pt2_from_pt5(pt5, use_lip=True):
    """Parsing the 2 points according to the 5 points, which cancels the
    roll."""
    if use_lip:
        pt2 = np.stack([(pt5[0] + pt5[1]) / 2, (pt5[3] + pt5[4]) / 2], axis=0)
    else:
...
def parse_pt2_from_pt9(pt9, use_lip=True):
    ...
    ['right eye right', 'right eye left', 'left eye right', 'left eye left',
    'nose tip', 'lip right', 'lip left', 'upper lip', 'lower lip']"""
    if use_lip:
        pt9 = np.stack(
            [
...
def parse_bbox_from_landmark(pts, **kwargs):
    ...
    bbox_rot = bbox.copy()
    R = np.array([[np.cos(angle), -np.sin(angle)], [np.sin(angle), np.cos(angle)]], dtype=DTYPE)
