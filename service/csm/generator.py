...
class Generator:
    ...
    def _tokenize_text_segment(self, text: str, speaker: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """Tokenize text segment with caching optimization for reduced
        latency."""
        # Check cache first
    ...
    def generate_stream(
        ...
        """Generate audio in a streaming fashion, optimized for lower latency
        to first chunk."""
        ...
        with self._audio_tokenizer.streaming(1):
            ...
            if frame_buffer:
                ...
                if len(frame_buffer) < expected_frame_count:
                    padding_frames = [
                        torch.zeros_like(frame_buffer[0]) for _ in range(expected_frame_count - len(frame_buffer))
                    ]
        ...
        if stream:
            if output_file:
                ...
                for i, chunk in enumerate(
                    self.generate_stream(text, speaker, context, max_audio_length_ms, temperature, topk),
                ):
                    # Write to file
                    write_chunk(chunk)
...
def warmup_generator(
    gen: Generator,
    warmup_text: str = 'Hello, this is just a quick test warmup itll speed it up.',
    speaker_id: int = 0,
):
...
def load_csm_1b(device: str = 'cuda') -> Generator:
    """Load the CSM-1B model with extreme optimizations for real-time
    performance."""
    # Enable all CUDA optimizations
    torch.backends.cuda.matmul.allow_tf32 = True
    torch.backends.cuda.enable_flash_sdp(True)
...
def generate_streaming_audio(
        ...
        for audio_chunk in generator.generate_stream(
            ...
            if frame_count % 10 == 0:
                ...
                if total_audio_duration > 0:
                    ...
                    print(
                        f'Chunk {chunk_count}: {total_audio_duration:.1f}s audio in {elapsed:.1f}s '
    ...
    print(
        f'Min/Max inter-chunk latency: {min_inter_chunk_latency * 1000:.1f}ms / {max_inter_chunk_latency * 1000:.1f}ms',
    )
